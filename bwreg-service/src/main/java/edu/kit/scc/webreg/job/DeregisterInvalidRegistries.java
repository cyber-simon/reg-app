...
public class DeregisterInvalidRegistries extends AbstractExecutableJob {
	...
	public void execute() {
		...
		if (! getJobStore().containsKey("service_short_name")) {
			logger.warn("DeregisterInvalidRegistries Job is not configured correctly. service_short_name Parameter is missing in JobMap");
			return;
		}
		...
		logger.info("Starting Deregister Invald Registries for {}", ssn);
		...
		try {
			...
			List<RegistryEntity> registryList = registryService.findByServiceAndStatus(ssn, RegistryStatus.INVALID, 
					new Date(System.currentTimeMillis() - lastUpdate), limit);
			
			if (registryList.size() == 0)
				logger.debug("No invalid registries found");
			
			for (RegistryEntity registry : registryList) {
				try {
					logger.info("Deregister Registry {}", registry.getId());
					registerUserService.deregisterUser(registry, "invalid-reg-job");
