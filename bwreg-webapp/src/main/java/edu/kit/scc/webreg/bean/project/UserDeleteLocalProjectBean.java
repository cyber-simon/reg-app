...
@Named
@ViewScoped
public class UserDeleteLocalProjectBean implements Serializable {
	...
	public void preRenderView(ComponentSystemEvent ev) {
		...
		if (adminIdentity == null) {
			...
			if (!(ProjectAdminType.OWNER.equals(adminIdentity.getType()))) {
				throw new NotAuthorizedException("Nicht autorisiert");
			}
		}
	}
	...
	public String commit() {
		service.deleteProject(getEntity(), "idty-" + session.getIdentityId());
		
		return ViewIds.PROJECT_LOCAL_INDEX + "?faces-redirect=true";
	}
	
	public String cancel() {
		return ViewIds.PROJECT_LOCAL_SHOW + "?id=" + getEntity().getId() + "faces-redirect=true";
	}
