...
@Named
@ViewScoped
public class WizardExpireEmailAddressesBean implements Serializable {
	...
	public void preRenderView(ComponentSystemEvent ev) {
		if (! initialized) {
			setJobName("wizard_ExpireEmailAddresses");
			setScheduleName("wizard_ExpireEmailAddressesSchedule");
			setLimit(1L);
			setScheduleTiming("5m");

			jobClass = jobClassService.findByAttr("jobClassName", "edu.kit.scc.webreg.job.ExpireVerifiedEmailAddresses");
			if (jobClass != null) {
	...
	public void createJobClass() {
		...
		if (getJobName() != null)
			...
			jobClass.setName("wizard_ExpireEmailAddresses");
		jobClass.setSingleton(true);
		jobClass.setJobClassName("edu.kit.scc.webreg.job.ExpireVerifiedEmailAddresses");
		jobClass.setJobStore(new HashMap<String, String>());
		jobClass.getJobStore().put("limit", getLimit().toString());
	...
	public void createSchedule() {
		...
		if (getScheduleName() != null)
			...
			jobSchedule.setName("wizard_ExpireEmailAddressesSchedule");
